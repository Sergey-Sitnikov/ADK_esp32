...
#define EXAMPLE_STATIC_IP_ADDR "192.168.1.17"
...
#else
#define EXAMPLE_MAIN_DNS_SERVER "192.168.1.1" // CONFIG_EXAMPLE_STATIC_DNS_SERVER_MAIN
...
esp_err_t Upload(httpd_req_t *req)
{
  ...
  while (remaining > 0)
  {
    int read_bytes = httpd_req_recv(req, buf, sizeof(buf));
    if (read_bytes <= 0)
    {
      if (read_bytes < 0)
      {
        ESP_LOGE(TAG, "Error during receiving: %d", read_bytes);
      }
      break;
    }
  ...
  fclose(f);
  httpd_resp_sendstr(req, web().c_str()); //, "File uploaded successfully");
  return ESP_OK;
}
